  ...
  <section class="wrapper" style='margin-top: 40px; display: inline-block; width: 100%;'>
    ...
    <form id="form1" name="form1" method="post" action="hpenv2List">
      <input type="hidden" name="orderKeyword" id="orderKeyword"
        value="<c:out value="${fn:trim(searchVO.orderKeyword)}" />" /> <input type="hidden" name="id" id="id" value="" />
          ...
          <header class="panel_header">
            ...
            <div class="pull-right" style="padding-top: 10px">
              <ul class="list-unstyled" style="width: 100%">
                <input type="hidden" name="searchType" value="lcode">
                <li style="float: left;">
                
                <select id="selectlcode"
                  style="width: 140px; padding-left: 10px; padding-right: 20px" name="code2"
                  class="form-control m-bot15">
                    <option value="">::대분류 선택::</option>
                    <c:forEach var="courseList" items="${courseList}" varStatus="status">
                      <option <c:if test="${courseList.sn eq searchVO.searchKeyword}">selected</c:if>
                        value="${courseList.sn}"><c:out value="${courseList.title}" /></option>
                    </c:forEach>
                </select>
            ...
            <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <table class="table">
                  ...
                  <tbody>
                    <c:forEach var="list" items="${chapterList}" varStatus="status">
                      <tr>
                        <td><c:out
    ...
    <script>
          function fn_viewsvy(lcode, lec1) {
            if (parseInt(lec1) < 10) {
    ...
    <script>
          function fn_formGo() {
            document.form1.lcode.value = document.getElementById("selectlcode").value;
            document.form1.action = "lecture6Form";
    ...
    <script>
          function fn_readGo(_a, _b) {
            document.form1.lcode.value = _b;
            document.form1.action = "lecture6Read";
