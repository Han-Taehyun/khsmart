        ...
        if ($.isFunction($.fn.fullCalendar)) {
            ...
            $('#calendar').fullCalendar({
					...
					
                {
					...
					title: '<c:out value="${listview.hh}" />:0<c:out value="${listview.mi}" /> <c:out value="${listview.status}" /> / <c:out value="${listview.customerid}" /> / <c:out value="${listview.userid}" />',
					</c:if>
					
					<c:if test="${'0' ne listview.mi}">
					title: '<c:out value="${listview.hh}" />:<c:out value="${listview.mi}" /> <c:out value="${listview.status}" /> / <c:out value="${listview.customerid}" /> / <c:out value="${listview.userid}" />',
                    </c:if>
					...
					<c:if test="${listview.userid ne ''}">
						...
						<c:if test="${'휴무' eq listview.status}">
						color: '#333',
						url: "javascript:alert('휴무자 알림 [ ${listview.cumemo} ]')"
						</c:if>
...
window.open("vwcal?sn="+_a, "_blank", "toolbar=no,scrollbars=yes,resizable=yes,left=0,top=0,fullscreen=yes");
}	
</script>

<script>
function winpopup2(_a){
window.open("newcal?ymd="+_a, "_blank", "toolbar=no,scrollbars=yes,resizable=yes,left=0,top=0,fullscreen=yes");
}	
</script>
