...
<form id="form1" name="form1" method="post" action="B2BBoardList">
			...
			<div class="col-lg-12">
				<section class="box ">
					...
					<header class="panel_header">
						<h2 class="title pull-left">단체 공지사항 목록</h2>
						<div class="pull-right" style="padding-top: 10px">
							<ul class="list-unstyled">



								<li style="float: left; margin-right: 20px"><select
									id="selectlcode"
									style="width: 200px; padding-left: 10px; padding-right: 20px"
									name="code2" class="form-control m-bot15">
										<option value="">::단체 선택::</option>
										<c:forEach var="etcgolist" items="${etcgolist}"
											varStatus="status">
											<option
												<c:if test="${etcgolist.sn eq searchVO.code2}">selected</c:if>
												value="${etcgolist.sn}">${etcgolist.elsname}</option>
										</c:forEach>
								</select></li>
									...
									<tbody>
										<c:forEach var="listview" items="${listview}"
											...
											<tr>
												...
												<td><c:out value="${listview.wrid}" /></td>
...
<script>
	function fn_formSubmit() {
		document.form1.action = "B2BBoardList";
...
<script>
	function fn_formGo() {
		...
		document.form1.action = "B2BBoardForm";
...
<script>
	function fn_readGo(_a) {
		...
		document.form1.action = "B2BBoardRead";
		document.form1.submit();
	}
</script>
